...
const Headingpad = () => {
  ...
  const { initImageLoading, onImageLoaded, isPageLoading, resetImageLoading } = useUISlice();
  const isUILoading = isPageLoading || !ready;

  React.useEffect(() => {
    if (!ready) return;
    if (!el.current) return; // Guard: ensure ref is attached to DOM
  ...
  }, [i18n.language, t, ready]); // Re-run when language is ready/changes

  React.useEffect(() => {
    initImageLoading(HERO_IMAGE_COUNT);
    return () => {
      resetImageLoading();
    };
  ...
  return (
    <>
      {isUILoading && <Loading />}
      <section
        className={`hero-section flex items-center w-screen transition-opacity duration-200 ${
          isUILoading ? "opacity-0 pointer-events-none" : "opacity-100"
        }`}
